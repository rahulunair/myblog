---
title: "Qwen3.8-27B capacity on four Arc Pro B70s"
subtitle: "Measuring maximum concurrency under a ten-second p95 time-to-first-token SLO"
pagetitle: "Qwen3.8-27B capacity on four Arc Pro B70s"
date: "2026-08-21"
date-modified: "2026-08-22"
description: "AIPerf capacity measurements for Qwen3.8-27B on four Arc Pro B70s: C=22 for 2K/512 and C=7 for 8K/1K under a ten-second p95 first-token SLO."
...
> **Measured capacity:** I write request sizes as input/output tokens, so
...
> 12,288-token per-request limit and does not fit.
>
...
At eight requests in flight, the server produced at least as many output tokens
per second as it did at seven, and every request completed. Its 95th-percentile
time to first token (p95 TTFT) had already crossed ten seconds. Throughput did
not identify the latency limit.

The test ran Qwen3.8-27B on four Intel Arc Pro B70s. I used the model's
multi-token-prediction head, or MTP, to draft several tokens before the full
model verified them. At one request in flight, the server prefilled an 8K
prompt at roughly 4,000 tok/s and streamed the completion near 104 tok/s. Those
numbers establish the single-request baseline, but they do not measure
concurrent capacity.

For the 8K/1K reference workload, the tuned 12K configuration supports **seven
concurrent requests** under a service-level objective (SLO) of p95 TTFT below
ten seconds. I use `C` for client concurrency, the number of requests AIPerf
keeps in flight. Three independent C=7 trials measured 8.695, 8.697, and 8.702
seconds. Three C=8 trials measured 10.656, 10.665, and 10.671 seconds.

For the shorter 2K/512 workload, the same server supported **22 concurrent
...
The short workload also stopped at C=11 even though eleven requests came
nowhere near filling the token pool. Explaining that result requires separating
the model's recurrent state from token storage and checking the raw response
stream rather than trusting token counters alone.

## Part I: Fix the server configuration
...
## Part II: Define the workload and SLO

### Reproduce the C=1 baseline
...
The rerun was close to the Docker Hub result, which confirmed the expected
image, checkpoint, attention backend, and MTP path. It did not measure the
...
This definition ties the concurrency limit to one workload and one latency
SLO. Changing the percentile creates a different result. Every
maximum-passing workload at the model's native 262,144-token context exceeded
ten seconds at p99, so none also passes a p99 version of the same limit.

## Part III: Compare p95 latency with throughput

### The native configuration supports seven 8K/1K requests
...
Output throughput has no clear drop between C=7 and C=8. The p95 TTFT values
cross the SLO at C=8. Peak tokens per second therefore cannot be divided by an
input/output token pair to produce a concurrency limit.

### Measure three request sizes

The 8K/1K workload is my reference case. I also measured one shorter and one
longer workload to see how the limit changes with request size.
...
The 2K/512 passing trials span 1.656 seconds across seeds, but all three remain
below the SLO. The 8K/1K trials cluster within 19 ms. That repeatability applies
to these trials and does not justify using a single trial for another system.
...
Even eleven 2K/512 requests near their full 2,560-token input-plus-output
length fit well below the shared 262,144-token pool. SGLang had, however,
reduced the requested 64 active slots to nine during startup.

I tested whether the 262,144-token maximum context caused that reduction.
Lowering maximum context alone changed nothing, so I examined the model's
recurrent state and the allocator's nine-request limit.

## Part IV: Account for attention KV and recurrent state

### One request allocates two kinds of state
...
During decode, MTP proposes a block and the target scores it. Accepted tokens
extend both attention KV and recurrent state. Graph replay reduces the overhead
of repeated decode shapes, but it does not reduce the prompt computation in a
long prefill.
...
## Part V: Map each serving flag to its resource

### Each flag controls a different resource
...
The graph list controls decode execution. Capturing batches 1, 2, 4, and 8
gives those shapes a replay path without rebuilding the command stream on every
token. Adding 12, 16, 20, 24, and 25 gives those larger batches the same path. A
missing graph tier can fall back to a slower execution path. A captured tier
does not guarantee that its batch meets the latency SLO.
Prefill graphs remained disabled in every server setting here.

Two other settings affect scheduling and storage granularity.
...
### Choose a 12,288-token maximum context
...
## Part VI: Tune recurrent-state capacity and decode graphs

### Changing maximum context alone does not change C=11
...
*Figure 5. Maximum context alone does not change C=11. Larger recurrent-state pools and captured decode batches increase short-request concurrency; the tuned 12K configuration passes C=22 and has mixed results at C=23.*
...
## Part VII: Confirm both workloads on one configuration

### The tuned 12K configuration supports 22 short requests
...
### The same configuration supports seven 8K requests
...
The native and tuned results are:
...
*Figure 6. The tuned configuration raises the 2K/512 limit from C=11 to C=22; the 8K/1K limit remains C=7.*

C increased only after both recurrent-state capacity and decode graph coverage
increased. Lowering maximum context alone did not change C=11. The tuned
configuration also did not change the C=7 limit for the longer prompt.

### The 8K/1K workload is limited by prefill time
...
The matched trials confirm this. On the native configuration, C=7 measured
...
*Figure 7. The native and tuned 12K configurations both pass C=7 and fail C=8 for 8K/1K, with nearly identical p95 TTFT in every trial.*
...
## Part VIII: Validate generated output

### Compare server token usage with streamed text
...
Server usage alone does not confirm that text was generated. An endpoint could
return plausible counters without streaming generated text. I parsed every raw
...
Generation correctness is part of the capacity test. I discard a latency
result if a request is shortened, cancelled, fails, or reports tokens without
streaming generated text.

## Part IX: Repeat the concurrency search with AIPerf

### Use AIPerf to find adjacent concurrency values
...
sensitive to roughly the slowest four, where a waiting group becomes visible.
Separate processes vary prompt order, speculative acceptance, and scheduler
timing across the three trials.
...
*Figure 8. AIPerf finds adjacent concurrency values; three clean-cache trials on each side and generated-output validation determine whether a value is reported.*
...
## Part X: Set a request limit from the measurements

### Capacity includes the request size and server configuration
...
The roughly 4,000 tok/s prompt-reading result describes one request. It does not
measure how many overlapping requests stay within the latency SLO. On this
host, the measured homogeneous-workload limits are C=7 for 8K/1K and C=22 for
2K/512 on the tuned 12K configuration. A mixed-workload limit still requires a
mixed-workload measurement.
