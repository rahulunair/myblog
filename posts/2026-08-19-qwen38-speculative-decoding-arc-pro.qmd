...
authoring-contract:
  ...
  secondary: mechanism-heavy launch deep-dive
  audience: inference engineers who need to reproduce or audit the serving result
  depth: (R1 stakes, R3-R5 measurement and implementation with R3 remeasurement, R3 shipping-status close, staircase braided with spiral)
  evidence-plan: final matched table; historical tables labelled by protocol; executed correctness; device phases; physical pool accounting; graph-ready CSV appendix
  publication-status: local draft only; do not publish until the new figures and social card are rendered and reviewed
-->

A long-running coding agent does not care that speculative decoding is fast for
the first eight thousand tokens if it falls apart after the repository, tool
history, and working notes fill the prompt. That was the constraint in this
work: both speculative modes had to keep working while the target server held a
real 1,048,576-token pool.

The final curve has a crossover rather than a universal winner. On four Intel
Arc Pro B70s at tensor parallel four, MTP reaches **109.2 tok/s at 32K** and
...
Getting there required more than enabling two server flags. The MTP head was
being constructed from uninitialized packed tensors. An eight-query verify
kernel was 46 times slower than one query over the same KV. The MTP draft path
needed a native XPU multi-step backend. DFlash2 needed a pinned source identity,
a physical sliding-window KV ring, target-only graph capture, and a request
validation guard. Several plausible fixes either did nothing or made the server
slower.

This work was measured and implemented independently. The DFlash2 checkpoint
and mechanism come from Inco, while the SGLang XPU integration, memory
accounting, lifecycle fixes, and Arc Pro qualification described here are the
paths exercised by this release.

## Part I: Run the qualified system and define the number

### Running the qualified configuration
...
Here is the complete qualified DFlash2 launch. The render group expression lets
...
The image supplies the XPU environment defaults that were part of the
measurement: decode INT8 up to M=8, symmetric all-reduce up to 131,072 bytes,
...
probe became part of the last release failure described later, so the public
recipe deliberately avoids it.

### Defining the number before comparing the arms

Every headline decode number here is:
...
The final long-context campaign used these controls:
...
One sample per long cell is a rough serving curve, not a latency distribution.
The shorter DFlash2 qualification used ten reported prompts per cell and is
shown separately.

## Part II: Follow one speculative iteration through the real workers

### Following one speculative step
...
| product arm | SGLang algorithm | proposal mechanism | production draft width |
|---|---|---|---:|
...
### Keeping greedy verification exact
...
tokens are proposals, and the target selects every committed token. The
headline campaign uses temperature zero, so equality means exact token equality
at each position. Sampling requires rejection sampling against target and draft
probabilities. SGLang has that broader machinery, but it is outside the measured
greedy envelope in this post.

### Producing one MTP proposal from two 5,120-wide rows
...
boundary position plus seven new proposals. This is the precise reason that the
production flags say seven steps and eight draft tokens.
...
### Producing a DFlash2 block in one draft forward

The external DFlash2 checkpoint declares `DFlash2DraftModel` and the following
production shape:
...
### Walking the DFlash2 candidate lattice
...
hidden state, and the elementwise product gates the predecessor representation.
...
That fallback is correct and was the path present in this image. It is also a
clear optimization boundary for future XPU work, so the release asserts model
identity and candidate shape separately from any backend-specific top-k kernel.
...
*MTP spends seven small sequential forwards to build its chain; DFlash2 spends one block forward plus a learned path selection. Both pay the same target-verification contract.*

### Mapping the mechanisms to SGLang's worker loop

Both workers begin with a normal target prefill. After that, their control flow
is deliberately similar: prepare draft inputs, produce candidates, call target
verify, accept a prefix, commit recurrent state, and prepare the next draft
boundary. Sharing that contract lets both arms use the same target scheduler,
KV pool, request batching, and output path.
...
The exact release tree is pinned at commit `7a65e2e`. These are the useful
repository-relative entry points when reading the implementation beside this
walkthrough:
...
| XPU ring and lifecycle policy | `20-patches/spec/post/0008-xpu-dflash2-production.patch` |

## Part III: Make MTP correct, then make its XPU path scale

### Repairing the MTP checkpoint before tuning it

The first MTP server generated readable text and reported an average accepted
...
Candidate logging found token id zero in every draft slot. The draft seed was
NaN, and the NaN began inside the MTP layer. Its projection modules had shape
metadata but no loaded weight tensor.
...
The repair changed configuration rather than weight bytes. The ignore list now
...
This failure established the first qualification rule: coherent target output
does not prove that a speculative draft works. Acceptance and loaded module
identity have to be checked beside text.
...
Raising the decode caps to M=8 and 131,072 bytes moved the same development
point from 102.6 to 105.5 tok/s. It was a real improvement, but the long-context
curve remained broken. A flag being accepted at startup had not shown that its
kernel served the request.
...
The first diagnosis came from the Python stack. All 25 `py-spy` samples sat in
...
The decisive probe removed the model and held the KV bytes constant. At the
...
The second number made the partial victory clear. Verify attention was a large
bottleneck, but the draft path still grew with context.
...
Patch 0007 adds native XPU metadata and attention for draft decode and draft
...
contract is specific to a linear top-k-one chain.
...
Historical diagnostic rows after patch 0007 showed the improvement before the
...
The natural next hypothesis was that MTP's auxiliary head remained too slow.
Device phase timers disproved it.
...
The categories close within 0.75 ms. The timer-free request ran at 68.580 tok/s
with a 39.934 ms median step. The timer-enabled request fell to 58.206 tok/s, a
useful reminder that attribution instrumentation changes the number it is
measuring.

The Gated Delta recurrent commit was not the memory wall we suspected. Focused
probes put the Mamba commit near 0.215 ms and hidden-state materialization near
2.2 ms. The dominant current category was target verification. Gated Delta
state capacity still affects how many requests fit, but it did not explain the
decode slowdown.

Two more target-attention ideas lost under measurement. Folding two query
...
## Part IV: Make DFlash2 reproducible and production-stable

### Making the DFlash2 identity reproducible

DFlash2 arrived from an unmerged SGLang development line. A generated overlay
once contained the newer DFlash worker but the release image's older
`dflash.py`, which exported classic DFlash only. Looking at the worker suggested
the port was current; checking the model registry exposed the mixed tree.
...
overlay manifest, and the build imports the result before asserting that
...
cannot pass those gates under a DFlash2 name.

The active patch order is 0001, 0003, 0005, 0006, 0007, the pinned DFlash2
port, then production patch 0008. The first five additions enable the XPU
...
contains the physical DFlash2 ring, radix-tail repopulation, graph policy, cache
...
| required production target | **1,048,576 tokens** |
...
The production design allocates a private, page-aligned ring for each live
...
Target and draft graph capture together still reduced the target pool to
...
exact **1,048,576-token target pool**. The final image sets that policy by
default.
...
Several candidate fixes did not establish a graph-on solution:
...
Process survival alone was therefore insufficient.

The final policy avoids DFlash2 draft graphs entirely while retaining target
graphs. The draft is small enough that this is the stable side of the trade.
The cache hooks remain defensive if that policy is overridden. DFlash2 also
...
## Part V: Measure the complete serving path

### Keeping prefill separate from decode
...
One isolated full-attention layer at the TP4 production shape measured:
...
Several bounded prefill experiments were rejected:
...
### Reading the final matched curve

The final campaign uses the same 1M target pool and the same width of eight for
both arms. Each row below is one reported exact-length prompt after one warm-up.

| exact input / output | arm | TTFT | prefill | median TPOT | median decode | raw output | acceptance |
|---:|---|---:|---:|---:|---:|---:|---:|
...
![](qwen38-spec-decode/long-context-crossover.svg){fig-alt="Matched long-context decode on four Arc Pro B70 cards. MTP leads 109.2 to 71.4 tokens per second at 32K and 82.3 to 58.1 at 128K, the arms nearly meet at 256K, and DFlash2 leads 42.2 to 37.2 at 512K."}

*MTP wins the measured 32K and 128K cells, the two arms meet at 256K, and DFlash2 wins the 512K cell on the same target pool.*
...
The curve also corrects an appealing but incomplete story. DFlash2 can propose
a whole block in one learned forward, yet proposal cost alone does not set
throughput. At 32K and 128K, MTP accepts enough more tokens to win. At 512K,
DFlash2 accepts a slightly longer block and finishes each streamed token in
less time. Draft architecture, acceptance, target verification, and state
commit all participate in the result.

The two arms produced byte-identical greedy output at 32K, 128K, 256K, and
512K. This coherence check does not replace a broad correctness suite, but it
does prove that the matched performance rows describe the same completion.

### Checking the shorter matched distribution

Both arms also ran ten reported prompts after a ten-prompt warm-up at 8K and
16K. The MTP arm came from the final pushed image; both arms used the same
exact cached datasets, a cold radix cache, and the 1M target-pool settings:

| exact input / output | arm | concurrency | TTFT | prefill | median TPOT | median decode | actual output | acceptance |
|---:|---|---:|---:|---:|---:|---:|---:|---:|
...
| exact input | arm | TPOT p50 | TPOT p90 | TPOT p99 |
|---:|---|---:|---:|---:|
...
![](qwen38-spec-decode/short-tpot-percentiles.svg){fig-alt="TPOT percentile ranges for ten prompts per arm. At 8K, MTP and DFlash2 nearly overlap from p50 through p99. At 16K, MTP remains between 9.363 and 11.091 milliseconds while DFlash2 widens from 10.694 to 19.201 milliseconds."}
...
MTP is within **0.95% at 8K**, which this campaign treats as a tie, and leads
...
At concurrency four, `57.4 × 4` gives 229.8 tok/s, but the server actually
...
The reproducible older MTP results remain useful historical controls:
...
verify-optimization protocol and stays labelled as a development result. None
of those older rows replaces the matched ten-prompt result above.

### Separating historical controls from the matched campaign
...
The correctly ported historical DFlash2 arm reached 121.0 tok/s at 8K with a
2,048-token window, acceptance 6.282, and a 51.28 ms speculative step. That run
predated the final physical ring and matched long-context protocol. It remains
useful implementation history and does not belong on the same curve as the
current 105.0 tok/s ten-prompt result.

### Executing correctness rather than reading outputs

The broad executed-code gate predates the final matched campaign:
...
byte-identical. DFlash2 passed the HumanEval regression gate, while its MBPP
row remains outside the completed evidence.

This is why the post avoids the word “lossless.” Verification remains
...
## Part VI: Ship the qualified envelope and leave an audit trail

### Shipping the clean image without the diagnostics

The release overlay is rebuilt from the pinned `qwen3.8-27b-20260816` base and
the reproducible patch series. Production patch 0008 is generated separately
...
qualification logs. CPU-only image gates assert:
...
- all qualified environment defaults.
...
during the following mode switch. The evidence does not identify a card defect
or a specific runtime root cause, so the public instructions avoid the cold
generation probe. The image was first verified offline against the already
qualified overlay. After the release image and documentation were prepared, a
single guarded MTP-only server was started from the pushed tag, allowed to warm
normally through `/health`, and used for the 8K and 16K results above. It
completed all 40 requests, stopped gracefully without a mode switch, and left
all four devices in their normal state.

That incident belongs in the record because it changes operational guidance:
...
### Cataloguing the experiments that did not become defaults
...
| skip `empty_cache` during flush | small lifecycle probe passed, 8K gate failed | rejected |
| graph teardown and recapture | process survived probes, 8K fell to 62.407 and 46.230 tok/s | not the production policy |
...
Keeping these failures prevents the next investigation from repeating a
plausible idea whose end-to-end gate already failed.

### Stating what 1M support means
...
The largest measured prompt in the final matched campaign is 524,288 tokens.
...
throughput row is claimed.

Radix prefix caching is enabled in the product configuration. The matched
campaign deliberately flushed it and checked for zero cached prompt tokens so
each row measured full prefill. A separate radix-positive agentic workload was
not completed, so there is no cache-hit throughput claim in this post.
...
```csv
arm,input_tokens,output_tokens,concurrency,samples_after_warmup,ttft_ms,prefill_tok_s,median_tpot_ms,median_decode_tok_s,raw_output_tok_s,acceptance
...
The leader margin uses the trailing arm as its denominator. The 1.8 percent
256K margin is reported for completeness and treated as a tie in the prose.

#### Short matched qualification

```csv
arm,input_tokens,output_tokens,concurrency,samples_after_warmup,ttft_ms,prefill_tok_s,median_tpot_ms,median_decode_per_user_tok_s,derived_median_times_c,actual_output_tok_s,acceptance
...
```csv
protocol,arm,input_tokens,output_tokens,decode_tok_s,acceptance,step_ms,notes
...
### Leaving the next bottleneck visible

The result is a production-capable serving configuration with a full target
pool and two useful speculative modes. It does not turn 512K decode into a
short-context experience. At 512K, target verification still owns the largest
measured MTP phase, and prefill remains hundreds of seconds for a fully cold
half-million-token prompt.

The next bounded decode experiment is to split the 36.5 ms target verification
phase into full attention, Gated Delta recurrence, dense projections,
collectives, and state commit, then optimize the largest measured component.
For DFlash2, a shape-matched XPU top-16 implementation could remove the current
generic `torch.topk` selector fallback only if it preserves exact candidate ids
and scores. Radix-positive agent traffic and a graded long-context quality suite
remain separate qualification projects.

The transferable lesson is smaller than either algorithm. Measure the narrowest
operation that can disprove the current story. A module registry disproved the
claimed DFlash2 source. One fixed-byte attention probe disproved the scheduler
hypothesis. Device phases disproved the draft-head hypothesis. Physical slot
accounting disproved the idea that a logical sliding window had solved memory.
Those checks produced the flags and limits in the release; another round of
