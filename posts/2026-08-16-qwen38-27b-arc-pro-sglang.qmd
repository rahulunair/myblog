---
title: "Qwen3.8-27B on Intel Arc Pro: 2.82 to 59.5 tok/s"
subtitle: "An 18.2 GiB four-bit build that gets four-card speed out of two cards, mostly by routing the work onto oneDNN primitives that already existed."
pagetitle: "Qwen3.8-27B on Arc Pro: 59.5 tok/s"
date: "2026-08-16"
description: "How a four-bit Qwen3.8-27B build reached 59.5 tok/s on four Intel Arc Pro B70s and 37.8 tok/s on two B65s using existing oneDNN primitives."
...
tok/s** on four B70s as an 18.2 GiB four-bit build of my own. Very little of that
came from writing kernels. Most of it came from removing one that should never
have been running, fixing a routing bug that made a fast path invisible, choosing
a quantization group size, and letting oneDNN's existing primitives do the
matrix work.
...
**Follow-up:** Speculative decoding takes the same AWQ target to **109.2 tok/s
at 32K** and **82.3 tok/s at 128K** on four B70s with MTP, while DFlash2 leads
at 512K. The full crossover and the commands to reproduce it are in
[Speculative decoding at 512K on Intel Arc Pro](/posts/2026-08-19-qwen38-speculative-decoding-arc-pro.html).
...
The biggest wins were all unglamorous. The 14.8x on the FP8 checkpoint was a
missing dispatch branch and a routing bug. The largest four-bit step came from a
...
the 48 DeltaNet layers I expected to dominate decode were not where the time
went.
...
tok/s on B65 and 44.0 on B70. Leaving `--max-total-tokens` unset lets the server
size the KV pool from the memory it finds; pin it lower if you are sharing the
cards with something else. The `SYS_PTRACE` and unconfined seccomp flags are there for one optional
...
That is 25.3 to 41.7, and 14.8x on the FP8 checkpoint in total, without writing
a kernel.
...
I built it with
[`llm-compressor`](https://github.com/vllm-project/llm-compressor), using its AWQ
modifier with an MSE observer and `duo_scaling=False`. Calibration ran
sequentially by `Qwen3_5DecoderLayer` over 512 chat-templated sequences of 1,024
tokens: 60% code instructions from `codeparrot/self-instruct-starcoder` and 40%
general instructions from `HuggingFaceH4/ultrachat_200k`. W4A16 keeps the
activations in BF16; only the selected weights are packed to asymmetric INT4.
...
fewer bytes bought about nine percent more tokens per second: the byte model
found the opportunity and then badly mispriced it.*
...
Once it was quick enough to use every day, the question became where the rest of
the step was going, and whether the tools telling me were trustworthy.
...
Before I had B70 cards I worked out a range for them instead of a number, with a
deliberately wide gap between the two edges. Then the cards arrived and I
measured it. The prediction is worth showing because one half of it was wrong in
an interesting way.
...
What carried over from earlier models, and what the coding agents were and were
not good for.
...
Speed is easy to measure and easy to fake. These are the checks, including the
one where I had the wrong model in my head.
...
Back to the 35% of bytes that returned 9% of throughput. I spent a long time
treating that as a saving I had somehow dropped between the checkpoint and the
step, and looking for where it went. It turns out there was nothing to find. The
saving was never there to collect.
...
I have kept the byte-model reasoning where it is earlier in this post rather
than pruning it back to the parts that worked out. It earned its place: it found
the group-size problem and it found the BF16 DeltaNet projections. It also
over-promised here. Both halves are worth knowing, and a model that is useful
and wrong in known places beats one that has been quietly tidied.
...
That is slower than collecting one exciting number, but it makes the next model
faster to understand. And Qwen3.8 is now a local coding-agent backend I can
