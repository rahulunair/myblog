...
> **Scope of the numbers:** results here are on Intel Arc Pro B65 and B70, and
> every row says which. The two sets sit either side of a physical card swap on
> the same machine on 17 August; the image, the overlays and the checkpoint did
> not change. Measured read bandwidth was 582 GB/s on B65 and 598 GB/s on B70,
> both against an advertised 608, so these are achieved rather than spec figures.
> "Decode tok/s" means `1000 / TPOT` at the stated context and concurrency; it is
> not the benchmark's output-throughput field, which folds time-to-first-token
> into the same wall-clock interval.
